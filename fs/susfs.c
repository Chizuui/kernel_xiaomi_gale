...
/* ReSukiSU manager compatibility for SUSFS v1.5.5 on legacy kernels. */
void susfs_set_avc_log_spoofing(void __user **user_info)
{
	struct st_susfs_avc_log_spoofing info = { .err = -EOPNOTSUPP };

	if (copy_to_user(*user_info, &info, sizeof(info)))
		SUSFS_LOGE("failed to report unsupported AVC log spoofing\n");
}
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	strlcat(info->enabled_features, "CONFIG_KSU_SUSFS_SUS_PATH\n", sizeof(info->enabled_features));
#endif
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	strlcat(info->enabled_features, "CONFIG_KSU_SUSFS_SUS_MOUNT\n", sizeof(info->enabled_features));
#endif
#ifdef CONFIG_KSU_SUSFS_SUS_KSTAT
	strlcat(info->enabled_features, "CONFIG_KSU_SUSFS_SUS_KSTAT\n", sizeof(info->enabled_features));
#endif
#ifdef CONFIG_KSU_SUSFS_SPOOF_UNAME
	strlcat(info->enabled_features, "CONFIG_KSU_SUSFS_SPOOF_UNAME\n", sizeof(info->enabled_features));
#endif
#ifdef CONFIG_KSU_SUSFS_HIDE_KSU_SUSFS_SYMBOLS
	strlcat(info->enabled_features, "CONFIG_KSU_SUSFS_HIDE_KSU_SUSFS_SYMBOLS\n", sizeof(info->enabled_features));
#endif
#ifdef CONFIG_KSU_SUSFS_SPOOF_CMDLINE_OR_BOOTCONFIG
	strlcat(info->enabled_features, "CONFIG_KSU_SUSFS_SPOOF_CMDLINE_OR_BOOTCONFIG\n", sizeof(info->enabled_features));
#endif
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	strlcat(info->enabled_features, "CONFIG_KSU_SUSFS_OPEN_REDIRECT\n", sizeof(info->enabled_features));
#endif
	if (copy_to_user(*user_info, info, sizeof(*info)))
		SUSFS_LOGE("failed to report enabled features\n");
	kfree(info);
}
...
void susfs_show_variant(void __user **user_info)
{
	...
	if (copy_to_user(*user_info, &info, sizeof(info)))
		SUSFS_LOGE("failed to report variant\n");
}
...
void susfs_show_version(void __user **user_info)
{
	...
	if (copy_to_user(*user_info, &info, sizeof(info)))
		SUSFS_LOGE("failed to report version\n");
}

void susfs_start_sdcard_monitor_fn(void)
{
	/* The v1.5.5 4.19 patch has no sdcard monitor implementation. */
}
